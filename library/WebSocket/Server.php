 ...
 * @author Chris
 */
class WebSocket_Server implements WebSocket_Observer {
    ...
    public function run() {
        ...
        $this->sockets->attach(new WebSocket_Socket($this, $this->master));
        ...
        while (true) {
            ...
            foreach ($changed as $resource) {
                ...
                if ($resource == $this->master) {
                    ...
                    $buffer = WebSocket_Functions::readWholeBuffer($resource);
    ...
    private function acceptSocket() {
        try {
            ...
            if ($client === false) {
                WebSocket_Functions::say('socket_accept() failed');
            }

            $this->sockets->attach(new WebSocket_Socket($this, $client));
...
//        $this->debug("dispatchMessage");
     ...
     */
    protected function addConnectionToUriHandler(WebSocket_Connection $user, $uri) {
        $url = parse_url($uri);
    ...
    public function onDisconnect(WebSocket_Socket $socket) {
    ...
    public function isAdmin(WebSocket_Connection $con) {
